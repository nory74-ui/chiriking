...
const CardView = ({ card, activeThemes, faceDown, selectable, onClick, highlight, size = 'normal', declaredName }) => {
  const baseClasses = "rounded-xl shadow-lg border-2 flex flex-col items-center justify-between relative transition-all duration-200 shrink-0 overflow-hidden";
  
  let sz;
  if (size === 'small') {
    sz = {
      card: "w-20 h-32 sm:w-24 sm:h-36 p-1.5 sm:p-2",
      id: "text-[8px] sm:text-[10px]",
      name: "text-[11px] sm:text-sm",
      attr: "text-[8px] sm:text-[10px] px-1 py-0.5",
      icon: 10,
      unit: "text-[6px] sm:text-[7px]",
      back: "text-[10px] sm:text-[12px]",
      declared: "text-[7px] sm:text-[9px] px-1.5 py-0.5 bottom-1.5"
    };
  } else if (size === 'large') {
    sz = {
      card: "w-32 h-48 sm:w-40 sm:h-64 p-3 sm:p-4",
      id: "text-xs sm:text-sm",
      name: "text-xl sm:text-2xl",
      attr: "text-[10px] sm:text-xs px-2 py-1",
      icon: 14,
      unit: "text-[8px] sm:text-[10px]",
      back: "text-[16px] sm:text-[20px]",
      declared: "text-xs sm:text-sm px-3 py-1 bottom-4"
    };
  } else {
    sz = {
      card: "w-24 h-36 sm:w-28 sm:h-44 p-2 sm:p-2.5",
      id: "text-[9px] sm:text-[11px]",
      name: "text-sm sm:text-lg",
      attr: "text-[9px] sm:text-[11px] px-1.5 py-0.5",
      icon: 12,
      unit: "text-[7px] sm:text-[8px]",
      back: "text-[12px] sm:text-[16px]",
      declared: "text-[9px] sm:text-[11px] px-2.5 py-0.5 bottom-3"
    };
  }
...
export default function App() {
  ...
  const executeDoubtPlay = (declaredPref) => {
    ...
    updateGame(prev => {
      const tid = getSafeTheme(prev.currentThemeId || pendingTheme, activeThemes);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (game.mode === 'daifugo') {
        ...
        updateGame(prev => {
          const tid = getSafeTheme(prev.currentThemeId || cpuTheme, activeThemes);
          return {
            ...prev, 
  ...
  if (!scriptsReady) return <div style={pageBaseStyle}><div className="text-white font-bold animate-pulse">Loading Assets...</div></div>;

  if (step === 'MENU') return (
    <div style={pageBaseStyle} className="p-6 text-center text-white font-sans relative">
      ...
      <div className="z-10 w-full max-w-sm space-y-10 animate-fade-in">
        ...
        <div className="space-y-6">
          <section className="bg-emerald-950/50 p-5 rounded-2xl border border-emerald-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-xs sm:text-sm text-emerald-100 ml-2">
              ...
              <li>場に出ているカードの「勝負属性」の数字より、<strong>大きい数字</strong>を持つカードを出せます。</li>
  ...
  if (step === 'PLAYER_SELECT') return (
    <div style={pageBaseStyle} className="p-6 text-white text-center">
      <div className="bg-emerald-800/80 border border-emerald-500/20 p-8 sm:p-10 rounded-[48px] max-w-md w-full shadow-2xl animate-fade-in backdrop-blur-md">
        <h2 className="text-2xl sm:text-3xl font-black text-emerald-300 mb-4 italic drop-shadow-md">何人で遊ぶ？</h2>
        <p className="text-[10px] sm:text-xs font-bold text-emerald-200/60 mb-8">{isOnline ? "参加を待つ人間の数を選択してください" : "残りの枠にはCPUが入ります（計4人）"}</p>
        <div className="flex flex-col gap-3 sm:gap-4 mb-8">
          {[1, 2, 3, 4].map(num => (
            <button key={num} onClick={() => { setPlayerCount(num); setStep('THEME'); }} className={`py-4 rounded-2xl font-black text-lg sm:text-xl border-2 transition-all duration-200 flex items-center justify-center gap-3 ${playerCount === num ? 'bg-emerald-500 text-emerald-950 border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.5)] scale-105' : 'bg-emerald-950/50 border-emerald-800 text-emerald-400 hover:border-emerald-500'}`}><Users size={20} /> {num}人</button>
          ))}
        </div>
        <button onClick={() => setStep('MENU')} className="text-xs sm:text-sm font-bold text-emerald-400/60 hover:text-emerald-300 underline">戻る</button>
      </div>
    </div>
  );
  ...
  if (step === 'THEME') return (
    <div style={pageBaseStyle} className="p-6 text-white text-center">
      <div className="bg-emerald-800/80 border border-emerald-500/20 p-6 sm:p-10 rounded-[32px] sm:rounded-[48px] max-w-lg w-full shadow-2xl animate-fade-in backdrop-blur-md">
        <h2 className="text-2xl sm:text-3xl font-black text-emerald-300 mb-4 italic drop-shadow-md">属性選択</h2>
        <p className="text-xs sm:text-sm font-bold text-emerald-200/60 mb-6">今回勝負する属性を<span className="text-white text-lg px-1">2つ</span>選んでください</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
          {Object.values(THEME_DEFS).map(t => {
            const isSelected = activeThemes.includes(t.id);
            return (
              <button 
                key={t.id} 
                onClick={() => {
                  if (isSelected) {
                    setActiveThemes(prev => prev.filter(id => id !== t.id));
                  } else {
                    if (activeThemes.length < 2) {
                      setActiveThemes(prev => [...prev, t.id]);
                    }
                  }
                }} 
                className={`p-3 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${isSelected ? 'bg-emerald-500 border-emerald-300 text-emerald-950 scale-105 shadow-[0_0_15px_rgba(16,185,129,0.5)]' : 'bg-emerald-950/50 border-emerald-800 text-emerald-400 hover:border-emerald-600'}`}
              >
                <t.icon size={24} />
                <span className="text-xs font-black">{t.name}</span>
                {isSelected && <Check size={16} className="absolute top-2 right-2 text-emerald-950" />}
              </button>
            );
          })}
        </div>
        <button 
          disabled={activeThemes.length !== 2}
          onClick={() => setStep('RULE')} 
          className={`w-full py-4 sm:py-5 rounded-3xl font-black text-xl sm:text-2xl shadow-lg transition-all ${activeThemes.length === 2 ? 'bg-emerald-400 hover:bg-emerald-300 text-emerald-950 active:scale-95' : 'bg-emerald-950 text-emerald-800 cursor-not-allowed'}`}
        >
          次へ
        </button>
        <button onClick={() => setStep('PLAYER_SELECT')} className="mt-6 text-xs sm:text-sm font-bold text-emerald-400/60 hover:text-emerald-300 underline">戻る</button>
      </div>
    </div>
  );
  ...
  if (step === 'LOBBY') return (
    <div style={pageBaseStyle} className="p-6 text-white text-center">
      <div className="bg-emerald-800/80 border border-emerald-500/20 p-8 sm:p-10 rounded-[48px] max-w-sm w-full shadow-2xl backdrop-blur-md animate-fade-in">
        <h2 className="text-2xl sm:text-3xl font-black text-emerald-300 mb-6 italic drop-shadow-md">待機室</h2>
        <div className="bg-emerald-950/80 p-4 sm:p-5 rounded-3xl mb-8 shadow-inner border border-emerald-900">
          <p className="text-[10px] font-black text-emerald-500 uppercase mb-2 tracking-widest">Room ID</p>
          <p className="text-4xl sm:text-5xl font-mono font-black text-emerald-300 tracking-widest drop-shadow-md">{roomId}</p>
        </div>
  ...
  if (step === 'RULE') return (
    <div style={pageBaseStyle} className="p-6 text-white text-center">
      <div className="bg-emerald-800/80 border border-emerald-500/20 p-8 sm:p-10 rounded-[48px] max-w-md w-full shadow-2xl backdrop-blur-md animate-fade-in">
        <h2 className="text-2xl sm:text-3xl font-black text-emerald-300 mb-8 italic drop-shadow-md">ルール選択</h2>
        <div className="flex flex-col gap-4 mb-10">
          <button onClick={()=>setMode('daifugo')} className={`py-5 sm:py-6 rounded-2xl font-black text-lg sm:text-xl border-2 transition-all flex flex-col items-center gap-1 sm:gap-2 ${mode==='daifugo'?'bg-emerald-500 text-emerald-950 border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.5)] scale-105':'bg-emerald-950/50 border-emerald-800 text-emerald-400'}`}>
            <span>基本モード</span>
            <span className={`text-[9px] sm:text-[10px] ${mode==='daifugo'?'text-emerald-900':'text-emerald-600'} font-bold`}>数字の大きさを競う王道ルール</span>
          </button>
          <button onClick={()=>setMode('doubt')} className={`py-5 sm:py-6 rounded-2xl font-black text-lg sm:text-xl border-2 transition-all flex flex-col items-center gap-1 sm:gap-2 ${mode==='doubt'?'bg-red-500 text-white border-red-400 shadow-[0_0_15px_rgba(239,68,68,0.5)] scale-105':'bg-emerald-950/50 border-emerald-800 text-emerald-400'}`}>
            <span>ダウトモード</span>
            <span className={`text-[9px] sm:text-[10px] ${mode==='doubt'?'text-red-100':'text-emerald-600'} font-bold`}>嘘をついて場を制する心理戦</span>
          </button>
        </div>
        <button onClick={handleStartButton} className="w-full bg-emerald-400 hover:bg-emerald-300 text-emerald-950 py-4 sm:py-5 rounded-3xl font-black text-xl sm:text-2xl shadow-lg active:scale-95 transition-all">決定！</button>
      </div>
    </div>
  );
  ...
  if (step === 'BOARD' && game) {
    ...
    const currentOrPendingThemeId = getSafeTheme(game.currentThemeId || pendingTheme, activeThemes);
    const td = THEME_DEFS[currentOrPendingThemeId];
    const currentMode = game.mode || mode;

    return (
      <div style={pageBaseStyle} className="p-0">
        <style dangerouslySetInnerHTML={{__html: `
          @keyframes fade-in { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
          .animate-fade-in { animation: fade-in 0.4s forwards; }
        ...
        {/* ヘッダー */}
        <div className="w-full bg-emerald-950/90 p-2 sm:p-3 flex justify-between items-center shrink-0 border-b border-emerald-500/20 shadow-md z-50 backdrop-blur-md">
          <div className="flex flex-col">
            <div className="font-black text-emerald-400 text-[9px] sm:text-[10px] italic tracking-widest flex items-center gap-1 uppercase"><MapIcon size={10} className="sm:w-3 sm:h-3"/> CHIRIKING</div>
            <div className={`text-[7px] sm:text-[8px] font-bold uppercase px-1.5 py-0.5 mt-0.5 rounded-sm inline-block ${currentMode === 'doubt' ? 'bg-red-600/20 text-red-400 border border-red-500/30' : 'bg-emerald-600/20 text-emerald-400 border border-emerald-500/30'} w-fit`}>{currentMode === 'doubt' ? 'DOUBT MODE' : 'BASIC MODE'}</div>
          </div>
          <div className="text-[10px] sm:text-xs font-black text-emerald-100 bg-emerald-800/50 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full border border-emerald-500/30 shadow-inner flex-1 mx-2 sm:mx-4 text-center truncate">{game.message}</div>
          <button onClick={() => setStep('MENU')} className="text-[8px] sm:text-[9px] bg-red-900/50 text-red-400 font-black px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg border border-red-800 hover:bg-red-800 transition-colors">EXIT</button>
        </div>

        <div className="flex-1 w-full flex flex-col items-center justify-center relative px-2 sm:px-4 pt-4 sm:pt-6 pb-2">
          ...
          {/* 中央エリア: 属性選択 / 場のカード */}
          <div className="flex-1 w-full flex flex-col items-center justify-center mt-8 sm:mt-10">
            {!game.currentThemeId ? (
              // 場が流れている時
              <div className="text-center z-10 animate-fade-in w-full max-w-xs sm:max-w-sm px-2">
                      ...
                      {activeThemes.map(tId => {
                        ...
                        return (
                          <button 
                            ...
                            className={`flex-1 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black flex flex-col items-center gap-1 sm:gap-2 border-2 transition-all duration-200 ${pendingTheme === TDef.id ? 'bg-emerald-400 text-emerald-950 border-emerald-300 scale-105 shadow-[0_5px_15px_rgba(52,211,153,0.4)]' : 'bg-emerald-950/50 border-emerald-800 text-emerald-500 hover:border-emerald-600'}`}
                          >
                            <TDef.icon size={20} className="sm:w-6 sm:h-6"/> 
                            <span className="text-xs sm:text-sm">{TDef.name}</span>
                          </button>
                        );
                      })}
                    </div>
              ...
              // 属性が決定している時
              td && (
                <div className="bg-emerald-950/60 p-4 sm:p-6 rounded-[32px] sm:rounded-[40px] border border-emerald-700/50 text-center mb-6 sm:mb-8 backdrop-blur-md shadow-xl animate-fade-in relative z-10 min-w-[200px] sm:min-w-[240px]">
                   <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-emerald-500 text-emerald-950 px-3 sm:px-4 py-1 rounded-full text-[8px] sm:text-[9px] font-black uppercase tracking-widest shadow-md flex items-center gap-1 border border-emerald-300">
                     <td.icon size={10} className="sm:w-3 sm:h-3"/> CURRENT THEME
                   </div>
        ...
        {/* 自分の手札セクション */}
        <div className={`h-56 sm:h-64 w-full flex flex-col border-t-2 p-2 sm:p-4 pb-2 sm:pb-4 shrink-0 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] z-50 backdrop-blur-2xl relative transition-colors duration-500 ${game.turn === actualViewIndex ? 'bg-emerald-800/90 border-emerald-400' : 'bg-emerald-950/95 border-emerald-900'}`}>
           <div className="flex justify-between items-center mb-2 sm:mb-4 max-w-lg mx-auto w-full relative px-2 sm:px-0 shrink-0">
             ...
             {game.turn === actualViewIndex && currentMode === 'daifugo' && game.currentThemeId && game.phase === 'WAITING' && (
               <button onClick={()=>handlePassBasic(actualViewIndex)} className="text-[10px] sm:text-xs font-black bg-slate-700 hover:bg-slate-600 text-white px-6 sm:px-8 py-1.5 sm:py-2 rounded-full border-b-2 sm:border-b-4 border-slate-900 active:scale-95 active:border-b-0 active:translate-y-1 transition-all shadow-lg">パスする</button>
             )}
             
             {game.phase === 'DOUBT_WINDOW' && game.pending?.playerIndex !== actualViewIndex && (
               <button onClick={()=>onResolveDoubt(actualViewIndex)} className="text-xs sm:text-sm font-black bg-red-500 hover:bg-red-400 text-white px-6 sm:px-10 py-2 sm:py-2.5 rounded-full border-b-2 sm:border-b-4 border-red-700 active:scale-95 active:border-b-0 active:translate-y-1 transition-all shadow-[0_5px_20px_rgba(239,68,68,0.4)] animate-pulse">ダウト！！</button>
             )}
                 ...
                 return (
                   <div key={c.id} className={`${game.turn === actualViewIndex && game.phase === 'WAITING' && selectable ? 'animate-[bounce_2s_infinite]' : ''}`} style={{animationDelay: `${Math.random()}s`}}>
                     <CardView 
                       ...
                       onClick={() => currentMode === 'daifugo' ? handlePlayBasic(actualViewIndex, c, getSafeTheme(game.currentThemeId || pendingTheme, activeThemes)) : handleDoubtPlayClick(c)} 
                       highlight={getSafeTheme(game.currentThemeId || pendingTheme, activeThemes)} 
                       size="small"
                     />
                   </div>
                 );
        ...
        {showDeclareModal && selectedCardToPlay && (
          <div className="fixed inset-0 bg-emerald-950/95 z-[400] flex flex-col items-center justify-center p-4 sm:p-6 text-center animate-fade-in backdrop-blur-xl">
            ...
            <div className="mb-4 sm:mb-8 flex justify-center scale-100 sm:scale-110 drop-shadow-2xl">
              <CardView card={selectedCardToPlay} activeThemes={activeThemes} highlight={getSafeTheme(game.currentThemeId || pendingTheme, activeThemes)} size="normal" />
            </div>
            ...
            <div className="w-full max-w-sm bg-emerald-900/50 p-3 sm:p-5 rounded-[24px] sm:rounded-[32px] border border-emerald-500/20 shadow-2xl">
              <div className="relative mb-3 sm:mb-4">
                <Search size={16} className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-emerald-400" />
                <input 
                  type="text" 
                  placeholder="都道府県を検索..." 
                  value={searchPrefectureQuery}
                  onChange={(e) => setSearchPrefectureQuery(e.target.value)}
                  className="w-full bg-emerald-950/80 text-white pl-10 sm:pl-12 pr-3 sm:pr-4 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl text-sm sm:text-base font-bold focus:ring-2 sm:focus:ring-4 ring-emerald-500/50 outline-none placeholder:text-emerald-700 shadow-inner"
                />
              </div>

              <div className="h-40 sm:h-48 overflow-y-auto rounded-xl sm:rounded-2xl bg-emerald-950/40 p-1.5 sm:p-2 space-y-1.5 sm:space-y-2 border border-emerald-800/50 scrollbar-hide">
                {PREFECTURES
                  ...
                  return (
                    <button 
                      ...
                      className={`w-full text-left px-3 sm:px-5 py-2.5 sm:py-4 rounded-lg sm:rounded-xl font-black transition-all flex justify-between items-center ${isTruth ? 'bg-emerald-600 text-white shadow-md' : 'bg-emerald-800/40 text-emerald-100 hover:bg-emerald-700'}`}
                    >
            ...
            <button 
              ...
              className="mt-4 sm:mt-8 text-xs sm:text-sm font-bold text-emerald-500 hover:text-emerald-300 underline tracking-widest"
            >
              キャンセル
            </button>
          </div>
        ...
        {game.phase === 'RESOLUTION' && game.doubtResult && (
          <div className="fixed inset-0 bg-emerald-950/95 z-[500] flex items-center justify-center p-4 sm:p-6 text-center animate-fade-in backdrop-blur-2xl">
            <div className={`p-6 sm:p-10 w-full max-w-sm rounded-[32px] sm:rounded-[48px] border-4 shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col items-center ${game.doubtResult.isTruth ? 'bg-emerald-600 border-emerald-300' : 'bg-red-600 border-red-300'}`}>
              <div className="bg-white/20 p-3 sm:p-4 rounded-full mb-3 sm:mb-4 backdrop-blur-sm">
                {game.doubtResult.isTruth ? <CheckCircle size={48} className="text-white sm:w-16 sm:h-16" /> : <XCircle size={48} className="text-white sm:w-16 sm:h-16" />}
              </div>
              <h2 className="text-4xl sm:text-5xl font-black mb-6 sm:mb-8 italic text-white uppercase drop-shadow-md tracking-tighter">
                {game.doubtResult.isTruth ? 'SUCCESS!' : 'OUT!'}
              </h2>
              
              <div className="flex flex-col items-center gap-3 sm:gap-4 mb-6 sm:mb-10 w-full">
                 <div className="flex justify-center scale-100 sm:scale-110 drop-shadow-xl"><CardView card={game.doubtResult.actual} activeThemes={activeThemes} highlight={game.doubtResult.themeUsed} size="normal" /></div>
              ...
              <div className="bg-black/20 w-full py-3 sm:py-4 rounded-xl sm:rounded-2xl mb-6 sm:mb-8 border border-white/10">
                <p className="text-white font-black text-base sm:text-lg">{game.players[game.doubtResult.loserId]?.name} がペナルティ</p>
